...
| Someone stopping or reconfiguring the gateway from a browser | Stop needs `listen.token` set and presented; the status page offers controls only then; Origin must be loopback or the page's own host; every reload, restart, stop and config change leaves a `manage` audit line; the YAML editor answers loopback requests only, since editing `command:` is running a program |
